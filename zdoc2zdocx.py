...
def detect_str(x: str) -> List[ZLabel]:
    ...
    while True:
        ...
        replace_str = [
            LeftSymbol + f"{idx+len(ZL)}" + RightSymbol
            for idx, m in enumerate(minimal_zlabel)
        ]
        ZL = ZL + Zlabels
    ...
    compiler_symbols = [
        int(c.replace(LeftSymbol, "").replace(RightSymbol, ""))
        for c in compiler_symbols
    ]
    merge_List = []
    ...
    while goon:
        ...
        for c in C_list:
            if check_children(c):
                ...
                compiler_symbols = [
                    int(cc.replace(LeftSymbol, "").replace(RightSymbol, ""))
                    for cc in compiler_symbols
                ]
                merge_List = []
                for i in range(len(compiler_symbols)):
                    if not other_content[i] == "":
